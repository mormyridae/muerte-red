...
plot.stanza <- function(n, a) {
    INDEX=toString(n)
    if(INDEX=="-1") {
        nodes <- read.csv("~/muerte-red/nodes.csv", header=T, as.is=T)
    links <- read.csv("~/muerte-red/edges.csv", header=T, as.is=T)
    ...
    if(INDEX=="-1") {
        visnet <- visNetwork(vis.nodes, vis.links, width="100%", height="1100px") #main=paste("death of narcissus")) %>% visPhysics(stabilization = FALSE)
    } else {
        visnet <- visNetwork(vis.nodes, vis.links, width="100%", height="1000px")#, main=paste("stanza ",toString(n+1),sep="")) %>% visPhysics(stabilization = FALSE)
    }
    return(visnet)
